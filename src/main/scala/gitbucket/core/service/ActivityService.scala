...
// TODO Don't use global execution context
import scala.concurrent.ExecutionContext.Implicits.global

trait ActivityService {

  def deleteOldActivities(limit: Int)(implicit db: Database): Int = run {
    (for {
      id   <- Activities.map(_.activityId).sortBy(_ desc).drop(limit).result.headOption
      rows <- Activities.filter(_.activityId <= id.bind).delete
    } yield rows).transactionally
  }

  def getActivitiesByUser(activityUserName: String, isPublic: Boolean)(implicit db: Database): List[Activity] = run {
    Activities
  ...
  def getRecentActivities()(implicit db: Database): List[Activity] = run {
    Activities
      .join(Repositories).on((t1, t2) => t1.byRepository(t2.userName, t2.repositoryName))
  ...
  def getRecentActivitiesByOwners(owners : Set[String])(implicit db: Database): List[Activity] = run {
    Activities
      .join(Repositories).on((t1, t2) => t1.byRepository(t2.userName, t2.repositoryName))
  ...
  def recordPushActivity(userName: String, repositoryName: String, activityUserName: String,
      branchName: String, commits: List[JGitUtil.CommitInfo])(implicit db: Database): Unit = run {
    Activities += Activity(userName, repositoryName, activityUserName,
      "push",
  ...
  def recordCreateTagActivity(userName: String, repositoryName: String, activityUserName: String, 
      tagName: String, commits: List[JGitUtil.CommitInfo])(implicit db: Database): Unit = run {
    Activities += Activity(userName, repositoryName, activityUserName,
      "create_tag",
